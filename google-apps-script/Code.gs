...
<div style="max-width:600px;margin:0 auto;">
  ...
  <div style="text-align:center;color:#aaa;font-size:11px;padding:12px 0;line-height:1.8;">
    ...
    Estimates for planning only. Verify figures before commercial use.
  </div>
</div></body></html>`
  ...
  // Attach the client-generated PDF report if provided
  if (data.pdfBase64) {
    const bytes = Utilities.base64Decode(data.pdfBase64)
    const blob  = Utilities.newBlob(bytes, 'application/pdf', data.pdfName || 'cashflow_report.pdf')
